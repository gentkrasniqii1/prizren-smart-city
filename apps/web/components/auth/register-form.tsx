...
export function RegisterForm() {
  ...
  useEffect(() => {
    ...
    const targetId = step === 1 ? 'first-name' : 'register-password';
    ...
    <AuthShell headline={t('panelHeadline')} body={t('panelBody')}>
      ...
      {step === 1 ? (
        <>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {t('accountType')}:{' '}
            <span className="font-medium text-foreground">{t('citizenRole')}</span>
          </p>
        </>
      ) : null}
      ...
      <div className="mt-5 flex items-center justify-between gap-3 lg:mt-4">
        <p className="text-caption text-muted-foreground" aria-live="polite">
          {t('registerStep', { current: step, total: TOTAL_STEPS })}
        </p>
        <div
          ...
          aria-valuenow={step}
          aria-label={t('registerStep', { current: step, total: TOTAL_STEPS })}
        >
          {Array.from({ length: TOTAL_STEPS }, (_, index) => (
            <span
              key={index}
              className={cn(
                'h-1.5 w-6 rounded-full transition-colors duration-normal ease-product',
      ...
      <form
        ...
        className="relative mt-6 space-y-4 lg:mt-5 lg:space-y-3"
        noValidate
      >
        <div key={step} className="motion-slide-in-right space-y-4 lg:space-y-3">
          {step === 1 ? (
            <>
              <div className="grid gap-4 sm:grid-cols-2 lg:gap-3">
                ...
                <div>
                  ...
                  <FieldError
                    ...
                    message={issueMessage(errors.lastName, t)}
                  />
                </div>
              </div>
              <div>
              ...
              <Button type="submit" className="w-full" size="lg" disabled={oauthBusy}>
                {t('registerContinue')}
              </Button>
            </>
          ) : (
            <>
              <div>
                ...
                <FieldError
                  ...
                  message={issueMessage(errors.password, t)}
                />
              </div>
              <div>
              ...
              <FieldError
                ...
                message={issueMessage(errors.acceptedTerms, t)}
              />
